...
async function _douyu_getRoomDetail(roomId) {
  ...
  const showStatus = room.show_status != null ? Number(room.show_status) : -1;
  const videoLoop = room.videoLoop != null ? Number(room.videoLoop) : -1;
...
globalThis.LiveParsePlugin = {
  ...
  async getPlayback(payload) {
    ...
    const rate = payload && payload.rate ? Number(payload.rate) : 0;
    const cdn = payload && payload.cdn ? String(payload.cdn) : "";
    if (!roomId) _douyu_throw("INVALID_ARGS", "roomId is required", { field: "roomId" });
    return await _douyu_getPlayback(roomId, rate, cdn);
  },
