        ...
        do {
            ...
            for item in dataReq.data {
                ...
                for realItem in item.data {
                    finalArray.append(LiveModel(userName: realItem.name ?? "", roomTitle: realItem.desc ?? "", roomCover: realItem.img ?? "", userHeadImg: realItem.avatar ?? "", liveType: .yy, liveState: "1", userId: "\(realItem.uid ?? 0)", roomId: "\(realItem.sid ?? 0)", liveWatchedCount: "\(realItem.users ?? 0)"))
                }
            }
            return finalArray
        }catch {
    ...
    public static func getLiveLastestInfo(roomId: String, userId: String?) async throws -> LiveModel {
        do {
            ...
            if let data = dataReq.data {
                ...
                throw LiveParseError.liveParseError("错误位置\(#file)-\(#function)", "错误信息：\(dataReq.resultCode)")
            }
        }catch {
            throw LiveParseError.liveParseError("错误位置\(#file)-\(#function)", "错误信息：\(error.localizedDescription)")
        }
                    ...
                    do {
                        ...
                        if let match = results.first {
                            ...
                            roomId = id
                             ...
                             throw LiveParseError.liveParseError("错误位置\(#file)-\(#function)", "错误信息：\("解析房间号失败，请检查分享码/分享链接是否正确")")
                        }
                    } catch let error {
                        throw error
                    }
